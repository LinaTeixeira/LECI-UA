...
void PersonSetAdd(PersonSet *ps, Person *p) {
  // COMPLETE ...!
  struct _PersonSet_* ps = (struct _PersonSet_*)malloc(sizeof(struct _PersonSet_));
  assert(ps != NULL);
  assert(PersonSetContains(ps, p->id));
...
PersonSet *PersonSetUnion(const PersonSet *ps1, const PersonSet *ps2) {
  ...
  // COMPLETE ...!
  while(ListGetCurrentPos(ps1->persons) <= ListGetCurrentPos(ps2->persons)){
