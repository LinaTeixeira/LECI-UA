...
int GraphRemoveEdge(Graph* g, unsigned int v, unsigned int w) {
  ...
  if (ListSearch(vertex_v->edgesList, &temp) == 0){
    return 0;
  }

  struct _Edge* e_v_w = ListGetCurrentItem(vertex_v->edgesList);
  ListRemoveCurrent(vertex_v->edgesList);
  free(e_v_w);
  ...
  if (g->isDigraph == 1) {
    // Only update in-degree
    vertex_w->inDegree--;
  } else {
    // UNDIRECTED → must also remove (w, v)

    struct _Edge temp2;
    temp2.adjVertex = v;
    temp2.weight = 0;

    if (ListSearch(vertex_w->edgesList, &temp2) == 1) {
      struct _Edge* e_w_v = ListGetCurrentItem(vertex_w->edgesList);
      ListRemoveCurrent(vertex_w->edgesList);
      free(e_w_v);

      vertex_w->outDegree--;
      // Do NOT modify numEdges again (already decremented once)
    } else {
      // This should NEVER happen if invariants are respected
      printf("Error: Graph invariant broken: missing (w,v) for undirected\n");
      return 0;
    }
  }
