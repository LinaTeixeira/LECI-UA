...
static int _treeGetHeightREC(const struct _BSTreeNode* root) {
  ...
  int leftH = _treeGetHeightREC(root->left);
  int rightH = _treeGetHeightREC(root->right);
...
void* BSTreeGetMin(const BSTree* header) {
  ...
  // ...                                                        completei!
  while(header->root->item != NULL){
    header = header->root->left;
  }
  return header->root->item;
}
