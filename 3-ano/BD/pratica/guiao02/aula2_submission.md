...
```
Entidades: Fornecedor, Encomenda, Produtos, Método pagamento, Tipo de Fornecedor

Atributos:
    Produtos: codigo, preço, IVA, nome, unidades em stock
    Fornecedor:código interno, nome, endereço, número fax, número identificação fiscal
    Tipo Fornecedor: ID, designação
    Método pagamento: ID, designação
    Encomenda : nº de encomenda, data

Relações:
    Fornecedor envia encomenda(1:N), Encomenda contem produtos(M:N)
...
```
    Fornecedor - Encomenda: um-para-N, participação obrigatoria por parte do fornecedor
    Encomenda - Produtos: muitos-para-muitos, participação obrigatoria por parte da encomenda
